...
> 设置好 `HS_SERVER`（服务器地址含端口）和 `HS_TOKEN_FILE`（Token 缓存文件路径）即可。

## 当前系统状态

!`
_HS_DIR="${OPENCLAW_HOME:-$HOME/.openclaw}/skills/home-signage"
source "$_HS_DIR/hs-config.sh" 2>/dev/null || { HS_SERVER="http://localhost:3000"; HS_TOKEN_FILE="$_HS_DIR/.token"; }

echo "服务器地址：$HS_SERVER"
if [ -f "$HS_TOKEN_FILE" ]; then echo "认证 Token：✅ 已缓存"; else echo "认证 Token：❌ 未配置（需先登录）"; fi
`
...
_HS_DIR="${OPENCLAW_HOME:-$HOME/.openclaw}/skills/home-signage"
source "$_HS_DIR/hs-config.sh" 2>/dev/null || { HS_SERVER="http://localhost:3000"; HS_TOKEN_FILE="$_HS_DIR/.token"; }
TOKEN=$(cat "$HS_TOKEN_FILE" 2>/dev/null)
